> **ARCHIVED (2026-07-09).** This v2.0 specification drifted ~5 months from
> the implementation and documented syntax that never shipped. It is kept for
> history only. The current, implementation-grounded specification is
> [BHDL_Complete_Specification.md](BHDL_Complete_Specification.md).
...
#### Parameter Value Domains (`where <param> in (...)`)

A string (or enum-like) parameter whose legal values are a **closed set**
declares that set in the entity's `where` clause. The set is the parameter's
*value domain*: an instantiation supplying anything outside it is a hard
error (**E0403**), not a silently-ignored value.

```bhdl
entity MOSFET(
    part_no: string = "2N7000",
    channel: string = "nmos"          // discrete FET polarity
)
    where channel in ("nmos", "pmos") // ← value domain for `channel`
{
    pin G: signal in;
    pin D: signal inout;
    pin S: signal inout;
...
Now the physics vocabulary is enforced at the instantiation site:

```bhdl
q1: MOSFET(part_no="FDN340P", channel="pmos")   // ✓ in domain
q2: MOSFET(part_no="FDN340P", channel="P")       // ✗ E0403: "P" is not one
                                                 //   of ("nmos", "pmos")
```

The check applies to **both** named and positional arguments (SKU aliases
pass the value positionally, e.g. `MOSFET("BSS84", "pmos", ...)`), and the
domain is resolved across imports — a board validates against the imported
entity's declared set. Multiple parameters may each carry a `where … in`
clause, comma-separated alongside any numeric `where` constraints.

**When to declare a domain.** Constrain a parameter when an unlisted value
is a *mistake* the toolchain would otherwise swallow silently (a wrong
footprint, an unrecognized polarity). Leave it open when unlisted values are
*valid but uncommon* and a sensible default applies — e.g. `LED(color)` maps
common colors to a forward voltage but orange/amber/UV LEDs are all real, so
its color is deliberately unconstrained.

> This entity-level `where` (after the parameter list, before `{`) is
> distinct from the *connection*-level `where` in §3.3, which carries
> physical constraints on a wire rather than a parameter's value domain.

#### Constructor Argument Validation

Every constructor argument must bind to a declared parameter, and its value
must satisfy any declared domain. Two error classes gate this — both are
hard errors that stop synthesis before a netlist is produced, so a design
never ships with an argument the part never received:

| Code  | Condition | Example |
|-------|-----------|---------|
| **E0402** | An argument names no declared parameter, or supplies more positional arguments than the entity has parameters. Emits a "did you mean" suggestion via edit distance. | `Res(1kΩ, tolernce=1%)` → *did you mean `tolerance`?* |
| **E0403** | An argument's value falls outside the parameter's declared value domain (`where <param> in (...)`). | `MOSFET(channel="P")` → not one of `("nmos", "pmos")` |

Unknown *entity types* are left to symbol resolution and never flagged here.
A small reserved namespace stamped by the toolchain itself — `supply_*` /
`i_supply` (supply synthesis metadata read by sign-off and ERC) and
`expansion_*` / `vpin_parent` (expansion provenance) — is exempt from E0402:
these are machine-authored attribute passthrough, not user parameters.
