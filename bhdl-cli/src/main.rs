...
enum Commands {
    ...
    Doc {
        ...
        /// Emit the board's PINMUX COMMITMENTS as a C header —
        /// firmware's copy of the solved alternate-function
        /// assignment (`mux__<field>` instance attributes + each
        /// chosen alternate's signal→pin map). "Record it in the
        /// bring-up code", made mechanical.
        #[arg(long)]
        mux_header: Option<PathBuf>,
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Doc { output, bom_only, budget_only, no_tree, no_patterns, mux_header }) => {
            cmd_doc(&source_file, &input_content, &cli.input, cli.no_elaborate, output, bom_only, budget_only, no_tree, no_patterns, mux_header).await?;
        }
...
/// The firmware's copy of the pinmux commitments: for every instance
/// with solved `mux__<field>` choices, one define per field naming the
/// chosen alternate and one per signal naming the physical pin — plus
/// the FIXED bindings of wired single-home fields, so the header is
/// the complete pin story of every peripheral the board uses.
fn emit_mux_header(netlist: &bhdl_netlist::Netlist, path: &Path) -> Result<()> {
    let mut lines: Vec<String> = Vec::new();
    lines.push("/* AUTO-GENERATED by `bhdl doc --mux-header` — the board's pinmux".into());
    lines.push(" * commitments (solved alternate-function assignment). Regenerate,".into());
    lines.push(" * never hand-edit: the board file is the source of truth. */".into());
    lines.push(String::new());
    let up = |s: &str| s.to_uppercase().replace(['.', '-'], "_");
    let mut any = false;
    ...
    for inst in insts {
        ...
        let mut fields: Vec<(String, String)> = inst
            .attributes
            .iter()
            .filter_map(|(k, v)| k.strip_prefix("mux__").map(|f| (f.to_string(), v.clone())))
            .collect();
        fields.sort();
        for (field, alt) in &fields {
            any = true;
            lines.push(format!(
                "#define BHDL_MUX_{}_{}_ALT \"{}\"",
                up(&inst.name),
                up(field),
                alt
            ));
            let prefix = format!("intf_bind_alt__{field}__{alt}__");
            let mut sigs: Vec<(String, String)> = module
                .attributes
                .iter()
        ...
        // story of the chip in one place)
        if !fields.is_empty() {
            let mut fixed: Vec<(String, String)> = module
                .attributes
                .iter()
                .filter_map(|(k, v)| k.strip_prefix("intf_bind__").map(|d| (d.to_string(), v.clone())))
                .collect();
            fixed.sort();
            for (dotted, pin) in fixed {
                lines.push(format!(
                    "#define BHDL_MUX_{}_{}_PIN {} /* fixed */",
                    up(&inst.name),
                    up(&dotted),
                    pin
                ));
    ...
    if !any {
        lines.push("/* no pinmux alternates on this board */".into());
    }
    lines.push(String::new());
    fs::write(path, lines.join("\n"))?;
    println!(
        "  {} pinmux commitments → {} ({} define(s))",
        "✓".green(),
        path.display(),
        lines.iter().filter(|l| l.starts_with("#define")).count()
    );
    Ok(())
}
...
async fn cmd_doc(
    ...
    // Pinmux commitments header (SoC arc increment 4): the solved
    // alternate-function assignment, emitted for firmware. Needs the
    // synthesized netlist (the solver runs there).
    if let Some(hdr) = mux_header {
        let netlist = verified_netlist(source_file, input_content, source_path, no_elaborate).await?;
        emit_mux_header(&netlist, &hdr)?;
    }
