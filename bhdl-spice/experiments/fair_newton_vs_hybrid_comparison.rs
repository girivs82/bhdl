/// Fair Newton-Raphson vs Ultimate Hybrid Comparison
/// 
/// Uses IDENTICAL circuit implementations and device models for both solvers:
/// - Same diode Shockley equation with exponential limiting
/// - Same MNA system building
/// - Same convergence tolerances
/// - Same test cases
/// 
/// Only difference: NR uses standard damping vs Hybrid uses 80% + smart damping

use nalgebra::{DMatrix, DVector};
use std::time::Instant;

// IDENTICAL element implementations for both solvers
...
// IDENTICAL diode model for both solvers
pub struct Diode {
    is: f64,
    vt: f64,
    voltage: f64,
...
// Pure Newton-Raphson Solver (using SAME device models)
...
impl NewtonRaphsonSolver {
    ...
    pub fn solve(&mut self) -> (Vec<f64>, f64, usize, f64) {
        ...
        println!("  [Newton-Raphson: Using SAME device models as hybrid]");
        
        // Standard Newton-Raphson with SAME tolerances as hybrid final phase
        let max_iter = 200;
        let tol = 1e-15;  // SAME as hybrid final tolerance
        let damping = 0.8;  // Standard conservative damping
    ...
    // IDENTICAL MNA system as hybrid solver
...
// Ultimate Hybrid Solver (using SAME device models)
...
impl UltimateHybridSolver {
    ...
    pub fn solve(&mut self) -> (Vec<f64>, f64, usize, f64, f64) {
        ...
        println!("  [Ultimate Hybrid: Using SAME device models as NR]");
        ...
        // Final solve with SAME tolerance as NR
    ...
    fn solve_final(&mut self, total_iterations: &mut usize) {
        let max_iter = 100;
        let tol = 1e-15;  // SAME tolerance as NR
        let damping = 0.8;  // SAME damping as NR
    ...
    // IDENTICAL MNA system as NR solver
    fn build_mna_system(&self) -> (DMatrix<f64>, DVector<f64>) {
        let n = self.num_nodes - 1;
        let m = self.source_currents.len();
...
fn main() {
    println!("=== FAIR NEWTON-RAPHSON vs ULTIMATE HYBRID COMPARISON ===");
    println!("Using IDENTICAL device models and circuit implementations");
    println!("Only difference: NR standard damping vs Hybrid 80% + smart damping\n");
    ...
    for &(test_name, vs, rs, is, vt) in &test_cases {
        // Newton-Raphson solve (SAME device models)
        ...
        // Ultimate Hybrid solve (SAME device models)
        ...
        let (hybrid_device_voltages, hybrid_id, hybrid_iters, hybrid_time, _oscillation) = hybrid_solver.solve();
    ...
    println!("\n📊 SUMMARY COMPARISON (IDENTICAL device models):");
    println!("  Newton-Raphson (Standard damping):");
    ...
    let iteration_ratio = if total_hybrid_iters > 0 { total_nr_iters as f64 / total_hybrid_iters as f64 } else { 0.0 };
    
    println!("\n🎯 FAIR PERFORMANCE COMPARISON:");
    if speed_ratio > 1.0 {
        println!("  Newton-Raphson is {:.1}x faster", speed_ratio);
    } else if speed_ratio > 0.0 {
        println!("  Ultimate Hybrid is {:.1}x slower", 1.0 / speed_ratio);
    }
    if iteration_ratio > 1.0 {
        println!("  Newton-Raphson uses {:.1}x fewer iterations", iteration_ratio);
    } else if iteration_ratio > 0.0 {
        println!("  Ultimate Hybrid uses {:.1}x more iterations", 1.0 / iteration_ratio);
    }
    println!("  Maximum error: {:.4}%", (total_v_error / n_cases).max(total_i_error / n_cases));
    
    println!("\n=== FAIR CONCLUSION ===");
    println!("Both solvers use IDENTICAL:");
    println!("  - Diode Shockley equation with exponential limiting");
    println!("  - MNA system building and matrix operations");
    println!("  - Final convergence tolerance (1e-15)");
    println!("  - Circuit topology and test cases");
    println!("");
    println!("Key difference: NR uses standard 0.8 damping throughout");
    println!("                Hybrid uses 80% fast ramping + adaptive smart damping");
    println!("");
    println!("This comparison shows the TRUE performance difference between algorithms,");
    println!("not implementation differences.");
}