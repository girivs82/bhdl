/// Hybrid Logarithmic Gradient Circuit Solver
/// 
...
/// 
/// Goal: 3-4x speedup while maintaining <1% error
...
// Fast history for phase 1 (simplified)
struct FastHistory {
...
impl FastHistory {
    fn new() -> Self {
        Self {
            sensitivities: VecDeque::with_capacity(5),
            convergence: VecDeque::with_capacity(5),
        }
    }
    ...
    fn add(&mut self, sensitivity: f64, converged: bool) {
        ...
        if self.sensitivities.len() > 5 {
            self.sensitivities.pop_front();
            self.convergence.pop_front();
        }
    }
    
    fn get_average_sensitivity(&self) -> Option<f64> {
        if self.sensitivities.is_empty() {
            None
        } else {
            Some(self.sensitivities.iter().sum::<f64>() / self.sensitivities.len() as f64)
        }
    }
...
// Accurate history for phase 2 (full algorithm)
#[derive(Clone)]
...
impl HybridController {
    ...
    fn switch_to_accurate_phase(&mut self) {
        self.phase = 2;
        // Start accurate phase conservatively
        self.accurate_rate = 0.005;
    }
    
    fn update_fast(&mut self, sensitivity: Option<f64>, converged: bool) {
        if let Some(sens) = sensitivity {
            let expected = 1.0 / self.vt;
            let ratio = sens / expected;
            
            if ratio > 5.0 || !converged {
                self.fast_rate = (self.fast_rate * 0.7).max(self.fast_min_rate);
            } else if ratio < 0.5 && converged {
                self.fast_rate = (self.fast_rate * 1.5).min(self.fast_max_rate);
            } else if converged {
                self.fast_rate = (self.fast_rate * 1.2).min(self.fast_max_rate);
            }
        } else if !converged {
...
impl HybridLogGradientSolver {
    pub fn new(num_nodes: usize, vt: f64) -> Self {
        Self {
            ...
            controller: HybridController::new(vt),
            phase_switch_ramp: 0.8,  // Switch at 80%
        }
    }
    ...
    pub fn solve(&mut self) -> (f64, f64, usize, f64) {
        ...
        while ramp_factor < 1.0 {
            ...
            if self.controller.phase == 1 {
                ...
                self.controller.update_fast(Some(sensitivity), converged);
            } else {
            ...
            if self.controller.phase == 1 && ramp_factor >= self.phase_switch_ramp {
                self.controller.switch_to_accurate_phase();
    ...
    fn solve_to_convergence_fast(&mut self, total_iterations: &mut usize) -> (bool, usize) {
        let max_iter = 20;  // Fewer iterations in fast phase
        let tol = 1e-10;    // Relaxed tolerance
        ...
        for iter in 0..max_iter {
            ...
            *total_iterations += 1;
            ...
            if let Some(x) = a.lu().solve(&b) {
                ...
                let damping = 0.8;  // Less conservative damping
    ...
    fn solve_to_convergence_accurate(&mut self, total_iterations: &mut usize) -> (bool, usize) {
        let max_iter = 30;  // Full iterations for accuracy
...
fn main() {
    println!("=== HYBRID LOGARITHMIC GRADIENT SOLVER ===");
    println!("Two-phase approach: Fast ramping (0-80%) + Accurate convergence (80-100%)\n");
    ...
    println!("{:>20} | {:>12} | {:>12} | {:>12} | {:>12} | {:>8} | {:>8} | {:>8}", 
             "Test Case", "SPICE Vd", "SPICE Id", "Hybrid Vd", "Hybrid Id", "Error %", "Iters", "Time ms");
    println!("{}", "=".repeat(110));
    ...
    for &(name, vs, rs, is, vt) in &test_cases {
        ...
        let mut solver = HybridLogGradientSolver::new(3, vt);
        ...
        let (vd_hybrid, id_hybrid, iters, time) = solver.solve();
        
        let v_err = ((vd_hybrid - vd_ref) / vd_ref * 100.0).abs();
        let i_err = ((id_hybrid - id_ref) / id_ref * 100.0).abs();
    ...
    println!("  Hybrid:    {:.2}% error, {:.1}ms time, {:.0} iterations", 
             total_error / n_cases, total_time / n_cases, total_iterations as f64 / n_cases);
    
    if total_time / n_cases < 55.5 {
        println!("\n✅ Speed improvement: {:.1}x", 55.5 / (total_time / n_cases));
    }
    if total_error / n_cases < 1.0 {
        println!("✅ Accuracy target achieved: <1% error");
    }
    
    println!("\n=== HYBRID APPROACH BENEFITS ===");
    println!("1. Fast initial ramping reduces wasted iterations");
    println!("2. Accurate final convergence maintains precision");
    println!("3. Phase transition at 80% balances speed and accuracy");
    println!("4. Adaptive to circuit complexity");
}