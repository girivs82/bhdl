//! Debug why different supply voltages affect GLACIER convergence

use anyhow::Result;
use bhdl_spice::{Circuit, ComponentModel, ElectricalLimits, GlacierSolver};

fn main() -> Result<()> {
    println!("=== Debugging Voltage Sweep for GLACIER ===\n");
    
    // Test different supply voltages
    let voltages = vec![3.0, 4.0, 5.0, 6.0, 7.0, 8.0, 9.0, 10.0, 11.0, 12.0];
        ...
        match glacier.analyze() {
            Ok(solutions) => {
                ...
                if !solutions.is_empty() {
                    ...
                    println!("    Regions: {:.1}%-{:.1}%, V_LED={:.3}V, I_LED={:.1}mA", 
