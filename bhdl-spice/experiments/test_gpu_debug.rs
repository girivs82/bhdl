//! Debug test for GPU solver
//! 
//! This test helps debug why the GPU solver is failing to converge by:
//! 1. Showing the circuit data being sent to GPU
//! 2. Showing the initial variable values
//! 3. Running a single iteration and showing the results
//! 4. Comparing with CPU solver results
...
fn create_simple_led_circuit() -> Circuit {
    ...
    let led_cathode = circuit.add_node("LED_CATHODE".to_string(), None);
...
fn debug_gpu_solver() -> Result<()> {
    
    println!("\n=== GPU Solver Debug Test ===\n");
    ...
    #[cfg(feature = "gpu")]
    {
        println!("GPU feature is enabled but this test is synchronous.");
        println!("The GPU solver requires async runtime.");
        println!("Showing what data would be sent to GPU:");
    ...
    #[cfg(not(feature = "gpu"))]
    {
        println!("GPU feature not enabled. Run with: cargo run --features gpu --bin test_gpu_debug");
    }
