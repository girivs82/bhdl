...
fn main() -> Result<()> {
    ...
    for (i, solution) in all_solutions.iter().enumerate() {
        ...
        let total_power = calculate_total_power(solution, &circuit);
    ...
    // MAESTRO returns a vector, typically with the best solution first
    if let Some(maestro_solution) = maestro_solutions.first() {
        println!("\n   MAESTRO selected solution:");
        println!("     Ramp: {:.1}%", maestro_solution.ramp * 100.0);
        
        // Convert to GlacierSolution format for compatibility
        let glacier_solution = bhdl_spice::GlacierSolution {
            ramp: maestro_solution.ramp,
            node_voltages: maestro_solution.node_voltages.clone(),
            branch_currents: maestro_solution.branch_currents.clone(),
            iterations: maestro_solution.iterations,
        };
        
        let maestro_power = calculate_total_power(&glacier_solution, &circuit);
        println!("     Total power: {:.3}W", maestro_power);
        
        // Verify selection is reasonable
        let i_total = glacier_solution.branch_currents.get("R1").copied().unwrap_or(0.0);
        ...
        for (node_name, voltage) in &glacier_solution.node_voltages {
            if let Some((idx, _)) = circuit.get_node(node_name) {
                initial_node_voltages.insert(idx, *voltage);
            }
        }
        
        for (branch_name, current) in &glacier_solution.branch_currents {
        ...
        let initial_conditions = bhdl_spice::AnalysisResult {
            ...
            iterations: glacier_solution.iterations,
        };
        ...
        save_comparison_results(&all_solutions, &glacier_solution, &result, &circuit)?;
    } else {
        println!("   ⚠️  MAESTRO did not return any solutions!");
    }
...
fn calculate_total_power(solution: &bhdl_spice::GlacierSolution, circuit: &Circuit) -> f64 {
    ...
    for (branch_name, current) in &solution.branch_currents {
        if let Some((branch_idx, _)) = circuit.get_branch(branch_name) {
            // Get the nodes connected by this branch
            if let Some((node1_idx, node2_idx)) = circuit.branch_nodes(branch_idx) {
                // Get node names
                if let (Some(node1), Some(node2)) = (circuit.get_node_by_id(node1_idx), circuit.get_node_by_id(node2_idx)) {
