//! Async test for GPU solver
//! 
//! This test properly tests the GPU solver with async runtime
...
#[cfg(feature = "gpu")]
use bhdl_spice::{
    glacier_gpu::{GlacierGpuSolver, gpu_data::GpuCircuitConverter},
};
...
#[cfg(feature = "gpu")]
async fn debug_gpu_solver() -> Result<()> {
    println!("\n=== GPU Solver Debug Test (Async) ===\n");
    ...
    match GlacierGpuSolver::with_config(config).await {
        Ok(gpu_solver) => {
            println!("GPU initialized successfully");
            
            // Debug: print what data would be sent to GPU
            ...
            match gpu_solver.solve(circuit).await {
                ...
                Err(e) => {
                    println!("\nGPU failed to converge: {}", e);
                    println!("This might be due to:");
                    println!("  1. Initial values not being set correctly");
                    println!("  2. Matrix assembly issues in the shader");
                    println!("  3. Solver parameters not matching CPU solver");
                    println!("  4. Shader compilation issues");
        ...
        Err(e) => {
            ...
            println!("This is expected if running without a GPU or WebGPU support");
        }
    }
...
#[cfg(not(feature = "gpu"))]
async fn debug_gpu_solver() -> Result<()> {
    println!("GPU feature not enabled. Run with: cargo run --features gpu --bin test_gpu_async");
    Ok(())
}
