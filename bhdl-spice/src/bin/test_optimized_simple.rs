/// Simple test of optimized logarithmic gradient solver
/// Just tests the basic diode circuit to get quick results

use nalgebra::{DMatrix, DVector};
use std::time::Instant;
use std::collections::VecDeque;

// Minimal element trait
...
// Simple optimized solver
pub struct SimpleOptimizedSolver {
...
impl SimpleOptimizedSolver {
    ...
    pub fn solve_simple(&mut self) -> (Vec<f64>, f64, usize) {
        ...
        // Simple ramping with optimizations
        let mut ramp_factor = 0.0;
        let mut ramp_rate: f64 = 0.02;  // Start more aggressive
        ...
        while ramp_factor < 1.0 {
            ...
            if converged && !self.nonlinear_elements.is_empty() {
                // Calculate sensitivity for adaptation
                ...
                // Adaptive rate based on sensitivity
                if sensitivity_history.len() >= 3 {
                    ...
                    if dv.abs() > 1e-9 {
                        ...
                        let expected = 1.0 / 0.026;  // 1/Vt
                        let ratio = sensitivity / expected;
                        
                        // Adjust rate based on sensitivity
                        if ratio > 3.0 {
                            ramp_rate *= 0.7;  // Slow down
                        } else if ratio < 0.5 {
                            ramp_rate *= 1.3;  // Speed up
                        } else {
                            ramp_rate *= 1.1;  // Slight speedup
                        }
...
fn main() {
    println!("=== SIMPLE OPTIMIZED LOGARITHMIC GRADIENT TEST ===");
    ...
    for (name, vs, rs, is, vt) in test_cases {
        let mut solver = SimpleOptimizedSolver::new(3);
        ...
        let (voltages, time, iterations) = solver.solve_simple();
        ...
        println!("\n{}: Vd={:.6}V (ref={:.6}V), Error={:.3}%, Time={:.1}ms, Iter={}", 
