/// Direct Binary Search Logarithmic Gradient Solver
/// 
/// Implements your exact algorithm:
/// - Start at ramp=0, voltage=0, error is negative (below target)
/// - Jump to ramp=1, voltage will overshoot, error becomes positive
/// - On sign change, go back halfway (ramp=0.5)
/// - Continue binary search based on error sign
/// 
/// The key: We use the diode's natural voltage as the implicit target
...
// Direct binary search state
struct DirectBinarySearch {
    // Binary search state
    ramp_low: f64,
    ramp_high: f64,
    current_ramp: f64,
    ...
    // Expected voltage (implicit target)
    expected_voltage: f64,
    
    iterations: usize,
}

impl DirectBinarySearch {
    fn new() -> Self {
        Self {
            ramp_low: 0.0,
            ramp_high: 1.0,
            ...
            expected_voltage: 0.6,  // Typical diode drop
            
            iterations: 0,
        }
    }
    
    fn initialize(&mut self, source_voltage: f64) {
        // Estimate expected diode voltage based on source
        self.expected_voltage = if source_voltage > 1.0 {
            0.6 + (source_voltage - 1.0) * 0.02  // Slight increase with voltage
        } else {
            source_voltage * 0.6  // Proportional for low voltages
        };
    }
    ...
    fn next_ramp(&mut self, current_voltage: f64) -> (f64, bool) {
        ...
        // First iteration - jump to middle
        if self.iterations == 1 {
            self.voltage_low = current_voltage;
            self.current_ramp = 0.5;
            return (self.current_ramp, false);
        }
        
        // Calculate error relative to expected
        let error = current_voltage - self.expected_voltage;
        let last_error = self.last_voltage - self.expected_voltage;
        
        // Debug output for first few iterations
        if self.iterations <= 10 {
            println!("    [Iter {}: ramp={:.4}, V={:.4}, error={:.4}]", 
                     self.iterations, self.current_ramp, current_voltage, error);
        }
        
        // Check for convergence
        if error.abs() < 0.001 || (self.ramp_high - self.ramp_low) < 0.0001 {
            return (self.current_ramp, true);
        }
        
        // Binary search logic
        if self.iterations == 2 {
            // Second iteration - establish bounds
            if error > 0.0 {
                // Overshot at 0.5
                self.ramp_high = self.current_ramp;
                self.voltage_high = current_voltage;
                self.current_ramp = (self.ramp_low + self.ramp_high) / 2.0;
            } else {
                // Still undershot at 0.5
                self.ramp_low = self.current_ramp;
                self.voltage_low = current_voltage;
                self.current_ramp = (self.current_ramp + self.ramp_high) / 2.0;
            }
        } else {
            // Subsequent iterations - true binary search
            if error > 0.0 {
                // Current voltage too high
                self.ramp_high = self.current_ramp;
                self.voltage_high = current_voltage;
            } else {
...
// Main solver
pub struct DirectBinaryLogGradientSolver {
    elements: Vec<Box<dyn Element>>,
    connections: Vec<(usize, usize, usize)>,
    node_voltages: Vec<f64>,
    source_currents: Vec<f64>,
    num_nodes: usize,
    search: DirectBinarySearch,
}

impl DirectBinaryLogGradientSolver {
    ...
    pub fn solve(&mut self) -> (f64, f64, usize, f64) {
        ...
        // Initialize search with source voltage
        self.search.initialize(source_voltage);
        
        // Binary search loop
        println!("  [Binary search for Vs={}, expecting Vd≈{}]", source_voltage, self.search.expected_voltage);
        ...
        loop {
            ...
            // Solve at this ramp
            self.solve_at_ramp(&mut total_iterations);
    ...
    fn solve_at_ramp(&mut self, total_iterations: &mut usize) {
        let max_iter = 50;
        let tol = 1e-12;
        let damping = 0.7;
...
fn main() {
    println!("=== DIRECT BINARY SEARCH LOGARITHMIC GRADIENT SOLVER ===");
    println!("Your exact algorithm: binary search on error sign changes\n");
    ...
    for &(test_name, vs, rs, is, vt) in &test_cases {
        ...
        let mut solver = DirectBinaryLogGradientSolver::new(3);
