...
fn main() {
    ...
    let sim_context = SimulationContext::new(0.001); // 1ms time step
    ...
    for step in 0..5 {
        ...
        // Create a new simulation context for this step
        let mut step_sim_context = SimulationContext::new(time_step);
        step_sim_context.current_time = current_time;
        
        // Create a new evaluation context for this step
        let mut step_eval_context = EvaluationContext::new(&step_sim_context);
        
        // Copy attributes from previous step
        for (name, value) in &eval_context.attributes {
            step_eval_context.set_attribute(name.clone(), value.clone());
        }
        
        // Update voltage level (simulate a square wave)
        let voltage = if step % 2 == 0 { 0.0 } else { 5.0 };
        step_eval_context.set_attribute("voltage_level".to_string(), RuntimeValue::Real(voltage));
        ...
        for attr_name in &attr_result.evaluation_order {
            if let Some(attr_decl) = attributes.get(attr_name as &str) {
                if let Some(expr) = attr_decl.value() {
                    match ExpressionEvaluator::evaluate(&expr, &step_eval_context) {
                        Ok(value) => {
                            println!("  {} = {:?}", attr_name, value);
                            step_eval_context.set_attribute(attr_name.clone(), value);
                        }
                        Err(e) => {
        ...
        if let Some(board) = source_file.boards().next() {
            for when_block in board.when_blocks() {
                if let Some(condition) = when_block.condition() {
                    match ExpressionEvaluator::evaluate(&condition, &step_eval_context) {
                            ...
                            for assignment in when_block.attribute_assignments() {
                                if let Some(name_token) = assignment.name() {
                                    ...
                                    if let Some(op) = assignment.op_token() {
                                        let current = step_eval_context.attributes.get(attr_name)
                                            .cloned()
                                                    ...
                                                    match op.text() {
                                                        "+=" => {
                                                            if let (Ok(curr_f), Ok(inc_f)) = 
                                                                ...
                                                                step_eval_context.set_attribute(attr_name.to_string(), new_val.clone());
                                                                println!("    {} += {:?} -> {:?}", attr_name, inc_value, new_val);
                                                            }
                                                        }
                                                        "=" => {
                                                            step_eval_context.set_attribute(attr_name.to_string(), inc_value.clone());
        ...
        // Update the main context with the step context values
        eval_context = step_eval_context;
